...
const getAbsoluteOriginPathInJs = function (
  filePath: string,
  extensions: string[]
): string | false {
...
const getOriginByAlias = function (
  filePath: string,
  alias: string[],
  extensions: string[]
): string | false {
...
export const getImport = function (
  ...
  const importedPath = getOriginByAlias(
    path.join(dirName, nodePath.node.source.value),
    [],
    extensions
  );
  return importedPath;
};
export const getRequire = function (
  nodePath: NodePath<namedTypes.Identifier>,
  dirName: string
) {
